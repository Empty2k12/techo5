...
// The playback codec accepts one format only. The ring is 768 x 4: on cronos the MediaTek I2S0dl1
// driver panics the kernel in its copy routine once the ring passes 12 KB, and 1024 x 4 is 16 KB.
const (
	playRate     = 48000
	playChannels = 2
	playBits     = 16
...
func newPlayCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			fmt.Fprintf(out, "playing %s for %.1fs on card %d device %d, ring %d x %d frames\n",
				what, secs, card, device, playPeriod, playPeriods)
